...
grid = {'n_estimators':[100,200,400,800,1000,1200,1500,1800,2000],'max_depth':[2,4,8],'max_features':['auto','sqrt','log2']}

#gs = GridSearchCV(gb, grid, cv=GroupKFold(n_splits=5))
#gs.fit(all_cols[0], all_cols[1], groups=groups)

#predefined splits
#gs = GridSearchCV(gb, grid, cv=ps.split(),scoring='accuracy')
gs = GridSearchCV(gb, grid, cv=ps.split(),scoring=['roc_auc','average_precision'],refit='roc_auc')
...
df_gridsearch.to_csv('model_hp_results/gradientboosting_all_features_hp.csv')
