...
def get_classification_metrics_test(testpath, model: keras.Model, test_set: tf.data.Dataset, fig, ax1, ax2, regre, kfold, split, guidelength, model_name='', dataset_name='', 
                               save=True):
    ...
    #ax1.plot(fpr, tpr, label='model %s, AUROC=%f' % (model_name, score), )
    #ax2.plot(precision, recall, label='model %s, AUPRC=%f' % (model_name, average_precision))
    ax1.plot(fpr, tpr, label='test set: AUROC=%f' % (score))
    ax2.plot(precision, recall, label='test set: AUPRC=%f' % (average_precision))
...
def get_regression_metrics_cd(model: keras.Model, test_set: tf.data.Dataset, regre, kfold, split, model_name='', dataset_name='', save=True):
    ...
    for i in range(len(outputs)):
        ...
        info.append(nuc_sequence)
        df1 = df[df['guide'] == nuc_sequence]
        gene_target = df1['gene'].values[0]
        #rank_gene = df1['t1_bin1_to_sum_bin14_rank_withr3'].values[0]
        #info = info + list(test_features[i]) #single value inputs
        info = info + [gene_target,outputs[i],labels[i]]
        test_predic.append(info)    
    
    #single_value_inputs = ['is_5UTR', 'is_CDS', 'is_3UTR', 'RNAseq', 'linearfold value']
    test_df = pd.DataFrame(test_predic, columns = ['spacer sequence', 'gene symbol','predicted_value','bin1_to_sum_bin14_avg_ratio_rank'])
    test_df['output rank'] = test_df['predicted_value'].rank()
...
def get_integrated_gradients(model, model_input, baseline=None, num_steps=50):
    if baseline is None:
        ...
        #average the input as baseline for continuous features
