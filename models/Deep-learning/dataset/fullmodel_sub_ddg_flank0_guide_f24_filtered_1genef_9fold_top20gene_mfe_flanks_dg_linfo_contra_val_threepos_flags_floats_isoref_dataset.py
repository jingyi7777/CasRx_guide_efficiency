...
def fullmodel_sub_ddg_flank0_guide_f24_filtered_1genef_9fold_top20gene_mfe_flanks_dg_linfo_contra_val_threepos_flags_floats_isoref_dataset(args):
    ...
    lin_seq_flanks_dict, lin_result_flanks_dict = parse_target_flanks_linearfold_fasta_into_dict_contrafold(flank_len = 0)
    linearfold_vals_target = [lin_result_flanks_dict[target_flanks] for target_flanks in dataframe['target_seq'].values] #native energy
    ddg = [] #energy required to unfold the guide binding region
    for jj in range(num_examples):
        ddg.append((linearfold_vals_target[jj]+0.29))
